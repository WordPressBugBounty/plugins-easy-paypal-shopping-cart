...
function wpepsc_plugin_settings_link($links) {
	...
	$premium_link = '<a target="_blank" href="https://wpplugin.org/downloads/easy-paypal-shopping-cart/">' . __('Get Pro', 'PTP_LOC') . '</a>';
...
function wpepsc_plugin_options() {
	...
	if (isset($_POST['update'])) {
		...
		if ( ! wp_verify_nonce( $nonce, 'wpepsc_form_post' ) ) {
	...
	<?php wp_nonce_field('wpepsc_form_post'); ?>
	...
	<center><a target='_blank' href="https://wordpress.org/support/plugin/easy-paypal-shopping-cart/reviews/?filter=5" class='button-primary' style='font-size: 17px;line-height: 28px;height: 32px;'>Leave a Review</a></center>
	<br />
...
function wpepsc_options($atts) {
	...
	$output .= "<input type='hidden' name='item_name' value='". esc_attr( $atts['name'] ) ."' />";
	$output .= "<input type='hidden' name='currency_code' value='$currency' />";
	$output .= "<input type='hidden' name='amount' value='". esc_attr( $atts['price'] ) ."' />";
